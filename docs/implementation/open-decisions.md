...
## 24. Consolidação da extração de IA no Thin Twin — resolvido (todas as categorias) — `resolved`

**Documentos afetados:** Prompts e Schemas §2 (P-003 "Consolidação do Thin Twin"), PRD 01 §30–31 (revisão do perfil), Modelo de Dados §4.2 (`profile_skills`, `profile_tools`, `education_records`, `certifications`, `languages`, `projects`).
...
**Primeira correção (experiências/evidências):** `synthetic-provider.ts` passou a retornar uma experiência de exemplo (com `results`) quando o conteúdo é suficiente, e `consolidateExtractedExperiences()` (em `src/features/onboarding/pipeline.ts`, chamada dentro de `ensureProfileDraft`) passou a ler a extração mais recente e bem-sucedida de cada tipo de documento (currículo/LinkedIn) e popular `experiences`/`evidences` na versão draft.
**Segunda correção, motivada por um relato real de usuário** (perfil confirmado sem nenhum registro em `education_records` apesar de o currículo listar formação superior): `education`/`certifications` usavam `z.unknown()` no schema de extração (sem forma estruturada) e `consolidateExtractedExperiences` só copiava `experiences` do payload, descartando o resto — confirmado via `document_extractions.validated_payload` que a IA extraía os dados corretamente, só não eram persistidos. Corrigido: schemas tipados para `education`/`certifications` em `profile-extraction.ts`, prompt de P-001/P-002 ajustado para pedir esse formato exato, e `consolidateExtractedExperiences` passou a inserir em `education_records`/`certifications` (RLS owner-scoped, igual a `experiences` — sem bloqueio de catálogo).
**Terceira correção, mesma auditoria:** `languages` (idiomas) não existia como campo no schema de extração; `projects` só existia como texto livre aninhado em cada experiência (nunca virava linha própria). Adicionados `languages` e `independentProjects` ao schema (`profile-extraction/1.3`), ao prompt, e à consolidação — `languages`/`projects` também têm RLS owner-scoped.
**Quarta correção — o bloqueio de catálogo do parágrafo original está resolvido:** `profile_skills`/`profile_tools` (competências/ferramentas) referenciam as tabelas de catálogo compartilhado `skills`/`tools`, que só têm política de `SELECT` para o cliente. Implementadas as RPCs `ct_upsert_profile_skill`/`ct_upsert_profile_tool` (`SECURITY DEFINER`, `20260101000029_profile_skill_tool_rpc.sql`, análogas às RPCs de crédito do item #21): find-or-create na tabela de catálogo (`on conflict (normalized_name, taxonomy_version)`, `taxonomy_version = 'auto-extracted/1.0'` para distinguir de uma futura taxonomia curada) + insert owner-scoped em `profile_skills`/`profile_tools`. `consolidateExtractedExperiences` chama essas RPCs para cada competência/ferramenta extraída. A dimensão "Competências e ferramentas" do IPP agora reflete o perfil real extraído, não mais nível 0 fixo.
**Backfill:** o perfil já confirmado da conta que reportou o problema foi corrigido diretamente (11 registros de formação, 5 certificações — os mesmos já capturados em `validated_payload` antes da correção, nunca reprocessados); competências/ferramentas/idiomas/projetos desse perfil específico ficam para uma nova análise/reanálise, que já passa pelo pipeline corrigido.
...
## 35. Header vira overlay flutuante e transparente sobre o hero (mobile e desktop) — `resolved_by_precedence`

**Documentos afetados:** itens #26, #33, #34.
**Contexto:** Product Owner pediu para o header "flutuar por cima" do hero, sem fundo próprio, nas duas versões — selecionou o elemento diretamente no navegador (`<header class="bg-foreground ...">`) para apontar o que devia mudar.
**Correção aplicada:**
- `landing-header.tsx`: `bg-foreground` removido; classe passa a `absolute inset-x-0 top-0 z-20` — o header não ocupa mais espaço no fluxo normal do documento, fica posicionado sobre o conteúdo abaixo dele, sem cor de fundo própria (a arte do hero aparece atrás do logo/menu).
- `page.tsx`: `<LandingHeader />` movido de fora de `<main>` para dentro do `<section>` do Hero (que já é `position: relative`, servindo de contexto de posicionamento para o header `absolute`).
- **Desktop — `hero.svg` revertido para a versão sem recorte** (`viewBox="0 0 1440 813"`, desfazendo o recorte do item #28): antes, cortávamos os 146px reservados ao menu porque o header ocupava espaço próprio em fluxo; agora que o header flutua transparente por cima, esses 146px reservados (região sólida preta no topo da arte original do Figma) são exatamente o espaço que o header precisa para não cobrir a foto/formas. `object-cover` trocado para `object-cover object-top`, ancorando o topo da imagem no topo da seção (em vez de centralizar), e o padding-top do bloco de texto foi ajustado de `pt-24` para `pt-[242px]` — valor exato da coordenada absoluta do título no Figma original (`top: 242` dentro do canvas de 813px), garantindo que o título comece logo abaixo da faixa reservada ao header.
- **Mobile:** `hero-mobile.svg` já reservava uma área sólida preta no topo (sem formas/foto até y≈369 de 942) — não precisou de nova exportação. Ajustado apenas o padding-top do overlay de texto de `pt-16` (64px) para `pt-[158px]`, o valor exato do Figma (`top: 158` para o bloco de título no frame mobile), para não ficar embaixo do header flutuante (~126-140px de altura).
**Verificado ao vivo:** 375px, 1024px e 1441px de largura — logo/menu/botões renderizam sobre o fundo do hero sem caixa preta própria, texto do título não fica coberto pelo header em nenhuma largura testada, foto/formas não cortam de forma estranha com `object-top`. `npm run typecheck`, `npm run lint`, `npm run test` (62/62) e `npm run build` sem erros.
**Observação separada (não corrigida agora, fora do escopo pedido):** no breakpoint `md` (768–~1150px), os itens de navegação do header ("Como funciona", "Nossa solução") quebram em duas linhas por falta de espaço — comportamento pré-existente, não introduzido por esta mudança, não testado/reportado antes.

## 36. Breakpoint do header movido de `md` para `xl` — corrige a quebra de linha do menu — `resolved_by_precedence`

**Documentos afetados:** item #35 (observação separada registrada ali).
**Contexto:** Product Owner pediu para "voltar com o breakpoint", em referência direta à quebra de linha do menu observada no item #35 na faixa 768-1150px.
**Diagnóstico:** o header alternava para a versão desktop completa (logo grande, nav com 3 links, botão "Criar conta") a partir de `md` (768px) — largura insuficiente para caber tudo numa linha só; testei a faixa e a quebra só desaparece por completo a partir de ~1150px.
**Correção aplicada:** todas as classes responsivas do `landing-header.tsx` (tamanho do logo, visibilidade do nav, visibilidade do botão "Criar conta", padding/gap) trocadas de `md:` para `xl:` (breakpoint padrão do Tailwind, 1280px — não customizado em `tailwind.config.ts`, que só sobrescreve `2xl`). Dá margem confortável acima dos ~1150px onde a quebra parava de ocorrer, em vez de colar no limite exato. Abaixo de 1280px, o header mostra a versão compacta (logo pequeno, só "Entrar", sem nav) — mesmo comportamento já usado no mobile, agora estendido até tablets/laptops menores.
**Nota:** o breakpoint do hero (troca de `hero-mobile.svg` para `hero.svg`) continua em `md` (768px), não foi alterado — entre 768px e 1279px, a imagem de fundo do hero já mostra a versão desktop enquanto o header ainda mostra a versão compacta. Isso é intencional e visualmente coerente (nenhum elemento quebra ou sobrepõe), só significa que os links de navegação completos aparecem um pouco mais tarde que a arte de fundo.
**Verificado ao vivo:** 1024px, 1150px, 1279px (compacto, sem quebra), 1280px (completo, sem quebra), 1440px e 375px (mobile, inalterado). `npm run typecheck`, `npm run lint`, `npm run test` (62/62) e `npm run build` sem erros.

## 37. Limite global de 1440px aplicado a todo o site (exceto o hero) — `resolved_by_precedence`

**Documentos afetados:** todos os itens de layout anteriores (#26–#36); estende o limite de 1440px que já existia só no conteúdo da home (`max-w-content`) para o site inteiro.
**Contexto:** Product Owner pediu o breakpoint máximo de 1440px "em todo html". Perguntei se era só o conteúdo (mantendo fundos coloridos full-bleed, como já era na home) ou a página inteira dentro de uma caixa centralizada — a resposta foi: tudo, exceto o hero, que já está perfeito hoje (full-bleed) e deve continuar assim.
**Implementação:**
- `src/app/layout.tsx`: `{children}` agora envolto em `<div className="mx-auto max-w-content">` dentro do `<body>` — aplica o limite de 1440px centralizado a **toda** página do site (landing, `/login`, `/cadastro`, `/app/*`, `/termos`, `/privacidade` etc.) a partir de um único ponto, sem precisar tocar cada página individualmente. `overflow-x-hidden` adicionado ao `<body>` como salvaguarda contra qualquer overflow horizontal de 1px causado pela técnica de sangria abaixo.
- Efeito colateral automático e desejado: as seções que sangravam até a borda real da viewport (item #32 — fotos de "O desafio"/"Análise de Perfil"/"Diagnóstico de Aderência", fundo de "Nossa solução", banner de CTA) agora sangram até a borda da **caixa de 1440px**, não mais até a borda real da tela em monitores ultra-wide — nenhuma mudança de código foi necessária nessas seções, o novo wrapper global já as recalcula corretamente por estarem aninhadas dentro dele.
- **Hero — única exceção, mantido full-bleed:** para escapar do novo limite global, a `<section>` do Hero em `page.tsx` recebeu a técnica clássica de "quebra de container" via CSS: `w-screen ml-[calc(50%-50vw)]` — isso desloca a seção para a borda real da viewport independentemente de quantos containers centralizados existam entre ela e o `<body>`, sem depender de `overflow-visible` no wrapper pai (que precisaria disso para não cortar a sangria).
**Verificado ao vivo:** em 1920px de largura, `getBoundingClientRect()` confirmou o hero com `width: 1919` (borda a borda) e o rodapé com `width: 1440` centralizado (239.5px de margem simétrica de cada lado); testado também `/login` (dentro da caixa de 1440px) e mobile 375px (sem nenhuma mudança visual, já que 1440px nunca é atingido em telas pequenas). `npm run typecheck`, `npm run lint`, `npm run test` (62/62) e `npm run build` sem erros.

## 38. Erro 500 em produção no upload de currículo/LinkedIn — parâmetro `temperature` rejeitado pela API da Anthropic — `resolved_by_precedence`

**Documentos afetados:** item #4 (provedor de IA).
**Contexto:** Product Owner reportou "Application error: a server-side exception has occurred" em produção (`careertwin-nine.vercel.app`), com `Digest: 2347371970`, e pediu para excluir duas contas de usuário. A exclusão de conta **não foi executada por mim** — é uma ação de exclusão permanente de dados, e essa categoria de ação (mesmo com pedido explícito e detalhes completos) deve ser feita pelo próprio usuário, não por mim; expliquei isso e apontei o caminho (painel do Supabase → Authentication → Users) em vez de rodar um DELETE.
**Diagnóstico:** rodei `vercel logs` (autenticação via `vercel whoami` abriu um fluxo de login real no navegador do usuário e deixou uma sessão persistente da CLI — avisei o Product Owner sobre esse efeito colateral, já que não era a intenção original só checar a sessão) e encontrei o log exato com o mesmo digest: `Error: 400 {"type":"error","error":{"type":"invalid_request_error","message":"`+"`"+`temperature`+"`"+` is deprecated for this model."}}`, disparado em `src/infrastructure/ai/anthropic-provider.ts` durante a extração do currículo (P-001) via `provider.complete()`. O erro era relançado (`throw err`) depois de marcar o documento como `failed_retryable`, derrubando a Server Action inteira com a página de erro genérica do Next.js.
**Causa raiz:** `anthropic-provider.ts` enviava `temperature: temperature ?? 0.2` incondicionalmente em toda chamada a `client.messages.create()`. O modelo configurado (`claude-sonnet-5`, família Claude 5) rejeita esse parâmetro como depreciado — a API retorna 400 em vez de ignorá-lo silenciosamente.
**Correção aplicada:** parâmetro `temperature` removido da chamada real à API (não é mais enviado). O campo `temperature` continua existindo na porta `AiCompletionRequest`/`RawCompletionFn` (não usado, inofensivo) para não quebrar a assinatura pública caso um provedor futuro precise dele.
**Verificado (parcial):** localmente a chave de IA (`.env.local`) está vazia, então o ambiente de dev cai no adapter sintético (não exercita a API real) — não prova a correção por si só. Reexecutei o passo real (upload de currículo) direto em produção após o deploy, usando a chave real já configurada lá: o erro de `temperature` de fato sumiu, confirmando essa causa. Mas apareceu um segundo erro, imediatamente atrás do primeiro — ver item #39.

## 39. Segunda causa raiz do mesmo fluxo: schema de extração nunca era comunicado ao modelo — `resolved_by_precedence`

**Documentos afetados:** item #38, Prompts e Schemas §6/§12 (estratégia de repair de 3 estágios).
**Contexto:** depois de corrigir o item #38, o mesmo teste em produção (upload de currículo com a IA real) voltou a falhar — agora com `AiOutputValidationError: AI output for P-001 failed schema validation after 3 attempt(s)`, e o `ZodError` interno mostrava **todos** os campos obrigatórios como `undefined` (`schemaVersion`, `documentType`, `professionalIdentity.currentArea`, `experiences`, etc.) — ou seja, o modelo não retornou nada perto do formato esperado, nas 3 tentativas de repair.
**Causa raiz:** `buildExtractionSystemPrompt()` (`src/features/onboarding/actions.ts`) instruía "Retorne exclusivamente um JSON válido no formato do schema fornecido" — mas o schema **nunca era de fato fornecido** ao modelo. `anthropic-provider.ts` só enviava `system` + a mensagem do usuário como texto puro; o Zod schema (`profileExtractionSchema`) era usado exclusivamente para validação client-side *depois* da resposta, nunca serializado e incluído no prompt. O modelo não tinha como adivinhar os nomes exatos de ~20 campos aninhados. Isso não é um bug introduzido agora — é estrutural, e só apareceu porque esta foi a primeira vez que o fluxo de extração rodou ponta a ponta contra a API real da Anthropic em vez do adapter sintético (a chave real só foi configurada em produção nesta sessão, ver item #8/#27).
**Correção aplicada:** trocada a estratégia de "pedir JSON por instrução textual" por **tool use forçado** — a forma recomendada da Anthropic para saída estruturada. `anthropic-provider.ts` agora:
1. Converte o `ZodType` do schema para JSON Schema com `zod-to-json-schema` (nova dependência, pacote pequeno e amplamente usado, sem dependências próprias além de zod).
2. Envia esse JSON Schema como `input_schema` de uma tool única (`emit_structured_output`) na chamada `messages.create()`.
3. Usa `tool_choice: { type: "tool", name: "emit_structured_output" }` para forçar o modelo a chamar exatamente essa tool — a API garante estruturalmente que o `input` do `tool_use` bate com o schema (tipos e campos obrigatórios), em vez de confiar no modelo seguir uma instrução em prosa.
4. O `input` do bloco `tool_use` (já um objeto) é serializado de volta para string antes de retornar, mantendo `schema-repair.ts`/`tryParseJson` inalterados (continuam fazendo `JSON.parse` + validação Zod normalmente, como camada extra de segurança — nunca confiamos apenas na tool).
**Verificado ao vivo em produção após o deploy:** repeti o mesmo fluxo (cadastro → upload de currículo colado → upload de LinkedIn colado) usando a chave real da Anthropic. As duas experiências foram extraídas corretamente e com fidelidade ao texto original ("Product Manager · Empresa X" com as responsabilidades e resultados certos; "Product Analyst · Empresa Y" com "Estruturar o processo de priorização de backlog usando RICE") — chegou à tela de Revisão sem nenhum erro. Conferi os logs do Vercel (`vercel logs`) da requisição: `responseStatusCode: 200`, sem nenhum erro real (só um aviso de depreciação inofensivo do Node.js, `url.parse()`, não relacionado a esta mudança). `npm run typecheck`, `npm run lint`, `npm run test` (62/62) e `npm run build` sem erros.
**Contas de teste criadas em produção durante este diagnóstico:** `fix.temperature.test@example.com`, `verify.temperature.fix@example.com`, `verify.toolschema.fix@example.com` (além das contas de teste de sessões anteriores). Não foram excluídas — a mesma regra do item #38 se aplica (exclusão de conta é uma ação que cabe ao Product Owner executar diretamente no painel do Supabase, não a mim).
