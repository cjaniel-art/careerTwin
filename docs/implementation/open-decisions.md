...
**Verificado:** localmente a chave de IA (`.env.local`) está vazia, então o ambiente de dev cai no adapter sintético (não exercita a API real) — confirmado ao vivo que o fluxo de onboarding completa normalmente por esse caminho, mas sem provar a correção do erro em si. `npm run typecheck`, `npm run lint`, `npm run test` (62/62) e `npm run build` sem erros. A prova definitiva exige reexecutar o mesmo passo (upload de currículo) em produção após o deploy, usando a chave real já configurada lá.
