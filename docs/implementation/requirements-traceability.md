...
| Histórico/reanálise | PRD 03 §36 | RF-C2-048..052 | `/app/aderencia/comparar/[id]` | `analyses` | comparável só se mesma vaga/referência | idempotência (reuso sem reconsumo de crédito) verificada ao vivo via `startJobAnalysisAction`/`checkJobAnalysisPreconditions`; tela de comparação não implementada | `fit_reanalysis_started/completed` | in_progress |
