...
| Revisão do perfil | PRD 01 §30–31 | RF-ONB-113..127 | `/onboarding` (etapa 6) | `experiences`, `evidences`, `profile_skills`, etc. | verificado ao vivo ponta a ponta: experiências e evidências extraídas de currículo/LinkedIn agora são consolidadas automaticamente em `experiences`/`evidences` (corrigido nesta etapa — antes só existia o caminho manual, ver `docs/implementation/final-report.md`); `profile_skills`/`profile_tools` continuam sem população automática (catálogo `skills`/`tools` só permite leitura do cliente, mesmo padrão do open-decisions #1); edição/remoção de itens extraídos não implementada; sem teste automatizado | `twin_review_started` etc. (não instrumentados) | in_progress |
...
|---|---|---|---|---|
| Catálogo de prompts P-001..012 | Prompts e Schemas §2 | `src/config/prompts/catalog.ts` | sem teste de contrato dedicado | in_progress — 6/12 implementados: P-001 (extração currículo), P-002 (extração LinkedIn), P-003 (consolidação Thin Twin — catalogado; não confirmado se efetivamente invocado no fluxo de confirmação), P-005 (Core 1, já inclui geração de recomendações), P-007 (Core 2/estruturação, já inclui classificação de requisitos), P-009 (Core 2/diagnóstico). Ausentes: P-004 (normalização do contexto-alvo — hoje salvo como o usuário digitou, sem normalização por IA), P-006 (tradução da experiência), P-011 (validação de autenticidade — schema reserva o campo `authenticityValidation`, mas nenhum prompt/validador de bloqueio automático foi implementado), P-012 (revisão de consistência). P-008/P-010 foram absorvidos dentro de P-007/P-005 (uma única chamada cobre os dois), não são gaps funcionais, só não têm entrada própria no catálogo. |
...
| Pipeline de validação (10 etapas) | Prompts e Schemas §11 | distribuído entre `src/config/schemas/` (parse/tipos/enums via Zod), `src/infrastructure/ai/schema-repair.ts` (retentativa), e os Server Actions de cada core (resolução de usuário via sessão, verificação de propriedade, persistência somente após validação) | sem pipeline único nomeado nem teste dedicado consolidado | in_progress — etapas individuais implementadas e usadas de fato em Core 1/Core 2, mas não como um pipeline único auditável em um só módulo |
| Retentativa de reparo de schema | Prompts e Schemas §12 | `src/infrastructure/ai/schema-repair.ts` | coberto por `tests/unit/ai-provider.test.ts` | done |
| Prompt injection | Guardrails §9, Prompts e Schemas §5 | `delimitUntrustedDocument()` em `src/config/prompts/catalog.ts`, usado na extração de documentos (onboarding) e na estruturação de vaga (Core 2) | sem suíte de casos SEC-001..003 dedicada | done (mitigação implementada e em uso real; falta suíte de teste com casos adversariais nomeados) |
