 ...
 * progresso/percentual (proibido pela spec). Itens de severidade "Alta" têm
 * o indicador numérico preenchido (em vez do círculo neutro), para saltar
 * aos olhos entre as 5 recomendações sem precisar ler o badge de cada uma.
 * O CTA abre o mesmo Sheet de plano de ação real (ActionPlanBody, com ações
 * rastreáveis desta análise de perfil) já usado no NextBestActionBanner do
 * relatório — não navega para /app/acoes (que mistura ações de todas as
 * análises do usuário).
 */
      ...
      <CardContent className="flex flex-1 flex-col gap-4 pt-2">
        {summary.length > 0 ? (
          <ol className="flex flex-col gap-3">
            {summary.map((action) => {
              const isPriority = action.severity === "Alta";
              return (
                <li
                  key={action.priority}
                  className={cn("flex items-center gap-3 rounded-md p-1.5", isPriority && "bg-destructive/5")}
                >
                  <span
                    className={cn(
                      "flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-semibold",
                      isPriority ? "bg-destructive text-destructive-foreground" : "bg-secondary text-foreground",
                    )}
                  >
                    {action.priority}
                  </span>
